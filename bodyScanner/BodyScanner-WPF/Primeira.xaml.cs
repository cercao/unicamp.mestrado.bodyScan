...
namespace Microsoft.Samples.Kinect.KinectFusionExplorer
{
    ...
    using Wpf3DTools;
    using Unicamp.Kinect.BodyScanner.Properties;
...
using ColorMine.ColorSpaces;
    ...
    /// </summary>
    public partial class Primeira : Page,  INotifyPropertyChanged, IDisposable
    {
        /// <summary>
        ...
        private System.Timers.Timer _timerImg;
        private bool _stopExporting;
        private int _exporterInterval = 10;

        private System.Timers.Timer exporterTimer;
        ...
        /// <summary>
        /// Lucas: Inicia stream de cor
        /// </summary>
        /// <param name="format">The resolution of image in depth stream</param>
        private void StartColorStream(DepthImageFormat format)
        {
            try
            {
                // Lucas: Inicia stream
                ...
                this.shadedSurfaceImage.Source = this.colorBitmap;
        ...
        private DateTime lastExporterTimestamp;
        ...
        private void WindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            catch (IndexOutOfRangeException)
            {
                ...
                this.statusBarText.Text = Unicamp.Kinect.BodyScanner.Properties.Resources.NoDirectX11CompatibleDeviceOrInvalidDeviceIndex;
                return;
            }
            catch (DllNotFoundException)
            {
                this.statusBarText.Text = Unicamp.Kinect.BodyScanner.Properties.Resources.MissingPrerequisite;
                return;
            }
            catch (InvalidOperationException ex)
            {
            ...
            System.Windows.Size depthImageSize = Helper.GetImageSize(DepthFormat);
            ...
            System.Windows.Size colorImageSize = Helper.GetImageSize(ColorFormat);
            ...
            this.lastExporterTimestamp = DateTime.Now;

            // Lucas: Seleciona modo de cor
            this.cbCaptureColor.IsChecked = true;

            // TODO: Verificar se rola usar o shadedSurface ARGB que já existe
            //shadedSurfaceFramePixelsArgbs
            //Thread thread = new Thread(delegate() { ExportPrint(this.shadedSurfaceFrameBitmap); });
            //thread.Start(); 


            _timerImg = new System.Timers.Timer();
            _timerImg.Interval = (3000);
            _timerImg.Elapsed += new ElapsedEventHandler((sender2, e2) => ExportPrint(sender2, e2, null, false));
            _timerImg.Enabled = true;

            _timer = new System.Timers.Timer();
            _timer.Interval = (3000);
            _timer.Elapsed += new ElapsedEventHandler((sender1, e1) => ExportPrint(sender1, e1 ,this.shadedSurfaceFrameBitmap, true));            
            _timer.Enabled = true;
            ...
            //        if ((DateTime.Now - this.lastExporterTimestamp).Seconds >= _exporterInterval)
            //        {
            //            ExportPrint();
            //        }
            //    }));
        ...
        private void ExportPrint(object sender1, ElapsedEventArgs e1, WriteableBitmap shadedSurface, bool eh3D)
        {
            ...
            Dispatcher.Invoke(new Action(() =>
            {
                ...
                BitmapEncoder encoderImg = new PngBitmapEncoder();

                // create frame from the writable bitmap and add to encoder
                if (shadedSurface != null)
                {

                    encoder.Frames.Add(BitmapFrame.Create(shadedSurface));
                    encoderImg.Frames.Add(BitmapFrame.Create(this.colorBitmap));

                    string time = System.DateTime.Now.ToString("hh'-'mm'-'ss", CultureInfo.CurrentUICulture.DateTimeFormat);

                    string path = "";
                    path = Path.Combine(Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName) + "\\Images\\Exported\\3D", "KinectSnapshot-" + time + ".png");
                    ...
                    try
                    {
                        ...
                        {
                            encoder.Save(fs);
                            // Update the last timestamp of exporter
                            this.lastExporterTimestamp = DateTime.Now;
                        }

                        // This is the bitmap we'll display on-screen
                        path = Path.Combine(Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName) + "\\Images\\Exported\\Color", "KinectSnapshot-" + time + ".png");
                        using (FileStream fs = new FileStream(path, FileMode.Create))
                        {
                            encoderImg.Save(fs);
                            // Update the last timestamp of exporter
                    ...
                    {
                        this.statusBarText.Text = string.Format("{0} {1}", Unicamp.Kinect.BodyScanner.Properties.Resources.ErrorSaveMesh, path);
                    }
                }
        ...
        private void FpsTimerTick(object sender, EventArgs e)
        {
            if (!this.savingMesh)
            {
                if (null == this.sensor)
                {
                    ...
                    this.statusBarText.Text = Unicamp.Kinect.BodyScanner.Properties.Resources.NoReadyKinect;
                }
                else
                {
                    ...
                    this.fpsText.Text = string.Format(
                        ...
                        Unicamp.Kinect.BodyScanner.Properties.Resources.Fps,
        ...
        private void StartStreams(DepthImageFormat depthFormat, ColorImageFormat colorFormat)
        {
            try
            {
                ...
                //this.StartColorStream(ImageFormat);
                ...
                // Passei - Cria espaço para construir
            ...
            this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.IntroductoryMessage);
        }
        ...
        private void TrackCamera()
        {
            ...
            if (!trackingSucceeded && 0 != this.successfulFrameCount)
            {
                ...
                if (!this.cameraPoseFinderAvailable)
                {
                    ...
                    this.ShowStatusMessageLowPriority(Unicamp.Kinect.BodyScanner.Properties.Resources.CameraTrackingFailed);
                }
                else
                {
            ...
            {
                // Bad tracking
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.ResetVolumeAuto);
        ...
        private bool FindCameraPoseAlignPointClouds()
        {
            ...
            {
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.PoseFinderNotEnoughMatches);
                return false;
            }
        ...
        private void UpdateCameraPoseFinder()
        {
            ...
            {
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.PoseFinderPoseHistoryFull);
            }
        }
        ...
        private void CheckNearMode()
        {
            if (null != this.sensor && this.nearMode != (this.sensor.DepthStream.Range != DepthRange.Default))
            {
                ...
                catch (InvalidOperationException)
                {
                    ...
                    this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.NearModeNotSupported);
                    this.nearMode = false;
                }
            }
        }
        ...
        private void CheckResetTimeStamp(long frameTimestamp)
        {
            if (0 != this.lastFrameTimestamp)
            {
                ...
                {
                    this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.ResetVolume);
                    this.resetReconstruction = true;
                }
            }
        ...
        private void ResetReconstruction()
        {
            ...
            if (null != this.volume)
            {
                ...
                {
                    this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.ResetFailed);
                }
            }
        ...
        private bool RecreateReconstruction()
        {
            ...
            {
                this.volume = null;
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.VolumeResolution);
            }
            catch (InvalidOperationException ex)
            {
                this.volume = null;
            ...
            {
                this.volume = null;
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.MissingPrerequisite);
            }
            catch (OutOfMemoryException)
            {
                this.volume = null;
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.OutOfMemory);
            }
        ...
        private void btnNext_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.SavingMesh);
                ...
                lock (this.volumeLock)
                {
                    ...
                    if (null == this.volume)
                    {
                        this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.MeshNullVolume);
                        return;
                    }
                ...
                if (null == mesh)
                {
                    this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.ErrorSaveMesh);
                    return;
                }
                ...
                dialog.Filter = "PLY Mesh Files|*.ply|All Files|*.*";
                    ...
                    // Defining RGB color
                    List<System.Drawing.Color> rgbColors = new List<System.Drawing.Color>();
                    ...
                    for (int i = 0; i < colors.Count; i++)
                    {
                        ...
                        rgbColors.Add(System.Drawing.Color.FromArgb(red, green, blue));
                    }
                    // Exporto grupos 
                    int width = (int)Math.Sqrt(rgbColors.Count); // read from file
                    int height = (int)Math.Sqrt(rgbColors.Count); // read from file
                    ...
                    using (MemoryStream memory = new MemoryStream())
                    {
                        for (int y = 0; y < height; y++)
                        {
                            ...
                            {
                                System.Drawing.Color c = rgbColors[contr];
                                bitmap.SetPixel(x, y, c);
                                contr++;
                            }
                        }
                        using (FileStream fs = new FileStream("C:/Temp/testeLucas.bmp", FileMode.Create, FileAccess.ReadWrite))
                        {
          ...
          //  bm.Save("Output\\out.bmp" ,System.Drawing.Imaging.ImageFormat.Bmp );
                   // bm.Save("C:/testeLucas.bmp", System.Drawing.Imaging.ImageFormat.Png);
            ...
            {
                this.ShowStatusMessage(Unicamp.Kinect.BodyScanner.Properties.Resources.ErrorSaveMeshOutOfMemory);
            }
            finally
            {
        ...
        private void Button_Click_1(object sender, RoutedEventArgs e)
        {
            ...
            // create frame from the writable bitmap and add to encoder
            encoder.Frames.Add(BitmapFrame.Create(this.shadedSurfaceFrameBitmap));

            string time = System.DateTime.Now.ToString("hh'-'mm'-'ss", CultureInfo.CurrentUICulture.DateTimeFormat);

            string path = Path.Combine(Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName) + "\\Images\\Exported", "KinectSnapshot-" + time + ".png");
        ...
        {
            // Finaliza threads para libera memorio
            this.StopStreams();
            ...
            try
            {
                ...
                lock (this.volumeLock)
                {
                    ...
                    // Garante que a próxima tela terá acesso ao mesh
                    mesh = this.volume.CalculateMesh(1);
                    Application.Current.Properties["mesh"] = mesh;
                }
                ...
                string path = Path.Combine(Path.Combine(Directory.GetParent(System.IO.Directory.GetCurrentDirectory()).Parent.FullName) + "\\Models", "Model.ply");
            ...
            finally
            {
                ...
                this.NavigationService.Navigate(new Segunda());
            }
        }
    }
