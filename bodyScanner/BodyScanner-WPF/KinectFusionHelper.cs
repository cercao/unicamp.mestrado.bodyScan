    ...
    public static class Helper
    {
        ...
        public static void SaveAsciiObjMesh(ColorMesh mesh, TextWriter writer, bool flipAxes)
        {
            ...
            for (int i = 0; i < vertices.Count / 3; i++)
            {
                ...
                rgbColors.Add(Color.FromArgb(255, 0, 0));
            ...
            for (int i = 0; i < rgbColors.Count / 3; i++)
            {

                // Classificate colors by group
                colorGroup.Add(Classify(rgbColors[i]));

                // Using LINQ for group by
                var numberGroups =
                    from n in colorGroup
                    group n by n into g
        ...
        public static int Classify(Color c)
        {
            ...
            if (brt < 0.2) return 1; //"Blacks";
            if (brt > 0.8) return 2; //"Whites";

            if (sat < 0.25) return 3; // "Grays";

            if (hue < 30) return 4; // "Reds";
            if (hue < 90) return 5;//"Yellows";
            if (hue < 150) return 6;// "Greens";
            if (hue < 210) return 7;//"Cyans";
            if (hue < 270) return 8;//"Blues";
            if (hue < 330) return 9;//"Magentas";
            return 10;//"Reds";
        }
        ...
        public static System.Windows.Size GetImageSize(DepthImageFormat imageFormat)
        {
            switch (imageFormat)
            {
                case DepthImageFormat.Resolution640x480Fps30:
                    return new System.Windows.Size(640, 480);

                case DepthImageFormat.Resolution320x240Fps30:
                    return new System.Windows.Size(320, 240);

                case DepthImageFormat.Resolution80x60Fps30:
                    return new System.Windows.Size(80, 60);
            }
        ...
        public static System.Windows.Size GetImageSize(ColorImageFormat imageFormat)
        {
            switch (imageFormat)
            {
                ...
                case ColorImageFormat.RgbResolution1280x960Fps12:
                    return new System.Windows.Size(1280, 960);
